...
export default function ActiveResultPage() {
  const router = useRouter();
  const { activeDealId, getDeal, updateDeal, currentUser } = useAppStore();
  const deal = activeDealId ? getDeal(activeDealId) : null;
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center space-y-3">
          <p className="text-slate-400">計算結果が見つかりません</p>
          <p className="text-slate-500 text-xs">新規案件登録から試算を実行してください</p>
          <Link href="/branch/deals/new">
            <Button variant="outline" className="mt-2">新規案件登録へ</Button>
          </Link>
        </div>
      </div>
    );
      ...
      {result && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-xs text-slate-400 flex items-center gap-2 font-medium uppercase tracking-wider">
                <TrendingUp className="h-4 w-4 text-blue-400" />A（受取予定利息合計）
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-xs text-slate-400 flex items-center gap-2 font-medium uppercase tracking-wider">
                <TrendingDown className="h-4 w-4 text-cyber-green" />B（再運用利息合計）
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-xs text-slate-400 flex items-center gap-2 font-medium uppercase tracking-wider">
                <Minus className="h-4 w-4 text-cyber-cyan" />A−B 現在価値合計
              </CardTitle>
            </CardHeader>
            <CardContent>
      ...
      {result && result.cashflows.length > 0 && (
        <Accordion type="single">
          <AccordionItem value="cashflows">
            ...
            <AccordionContent>
              <div className="overflow-x-auto rounded-lg" style={{ border: "1px solid rgba(26,58,92,0.4)" }}>
